...
const TestCard = ({test} : {test : testType}) => {
    return (
        <div data-aos="fade-up" data-aos-duration="1500">
            <div className="bg-[#0C0D0B] flex flex-col rounded-lg w-full border border-gray-700 p-4">
                
                <Image src={test?.photo} alt="test image" height={200} width={200} className="min-h-52 w-full h-56"></Image>
                <div className="p-5 space-y-2 md:space-y-3 flex flex-col ">
                    <span >
                        <h3 className="text-gray-100 text-2xl lg:text-3xl font-sansline-clamp-1 font-unbounded font-medium">{test?.name}</h3>
                    </span>

                    <p className="text-gray-300 text-sm lg:text-base line-clamp-5 flex-shrink ">{test?.details}</p>

                    <div className="flex justify-between">
                        <Link href={`/tests/${test._id}`}>
                            <GrLinkNext className="text-teal-500 text-2xl hover:translate-x-2 duration-150 cursor-pointer " />
                        </Link>
